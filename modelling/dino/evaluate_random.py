...
if __name__ == '__main__':
    # --- Keep their original argument parser setup ---
    ...
    parser.add_argument('--imagery_source', type=str, default='S', help='L for Landsat and S for Sentinel')
    # Note: I changed default imagery_source to 'S' since KidSat uses Sentinel
    parser.add_argument('--imagery_path', type=str, help='The parent directory of all imagery')
    parser.add_argument('--mode', type=str, default='spatial', help='Evaluating temporal model or spatial model')
    # Note: I changed default mode to 'spatial' since that's what we are doing
    ...
    # We remove the '--grouped_bands' argument because we will generate them randomly in the script now!
    
    # Add a new argument so you can control how many random tests to run from the command line
    parser.add_argument('--num_random_tests', type=int, default=10, help='How many random band combinations to test')
    
    args = parser.parse_args()

    # --- OUR NEW RANDOM LOOP LOGIC ---
    if args.mode == 'spatial':
        import random
        # Import the random module for band selection
        import csv
        # Import the csv module to log our results
        
        tested_combos = set()
        # Create an empty set to keep track of combinations we've already tried
        available_bands = list(range(1, 14)) 
        # Create a list of Sentinel bands 1 through 13
        
        # Open our CSV file to write the results
        with open('random_bands_mae_results.csv', mode='w', newline='') as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow(['Band_1', 'Band_2', 'Band_3', 'Fold_1', 'Fold_2', 'Fold_3', 'Fold_4', 'Fold_5', 'Avg_MAE'])
            # Write the header row so our columns are labeled
            
            # Start a loop that runs until we hit your requested number of tests
            while len(tested_combos) < args.num_random_tests:
                
                # Pick 3 random bands without replacement, and sort them so order doesn't matter
                combo = tuple(sorted(random.sample(available_bands, 3)))
                
                # If we've already tested this exact combination, skip to the next iteration
                if combo in tested_combos:
                    continue
                
                # Add this new combination to our tracking set
                tested_combos.add(combo)
                print(f"\n=====================================")
                print(f"Testing Random Bands: {combo}")
                print(f"=====================================")
                
                maes = []
                # Create an empty list to hold the MAE for the 5 folds
                
                # Loop through the 5 cross-validation folds (1, 2, 3, 4, 5)
                for i in range(5):
                    fold = i + 1
                    try:
                        # Call their original evaluate function, but pass our random 'combo' as a list
                        mae = evaluate(str(fold), args.model_name, args.target, args.use_checkpoint, args.model_not_named_target, args.imagery_path, args.imagery_source, args.mode, args.model_output_dim, grouped_bands=list(combo))
                        maes.append(mae)
                        # Add the successful MAE to our list
                    except Exception as e:
                        # If the evaluation crashes (e.g., missing file), print the error and append "Error"
                        print(f"Error on fold {fold}: {e}")
                        maes.append("Error")
                
                # Check if all 5 folds completed successfully (are numbers, not "Error")
                valid_maes = [m for m in maes if isinstance(m, float) or isinstance(m, np.float64)]
                
                if len(valid_maes) == 5:
                    avg_mae = np.mean(valid_maes)
                    # If all 5 worked, calculate the average MAE
                else:
                    avg_mae = "Error"
                    # Otherwise, mark the average as an error
                
                # Write a new row to the CSV with the 3 bands, the 5 fold scores, and the average
                writer.writerow([combo[0], combo[1], combo[2]] + maes + [avg_mae])
                
                # Force the computer to save the CSV to disk immediately so you don't lose data if it crashes
                csv_file.flush()
                
                print(f"Finished {combo} | Avg MAE: {avg_mae}")

    else:
        # If you accidentally run this in temporal or one_country mode, it throws an error.
        raise Exception("This random band script is only designed for spatial mode!")